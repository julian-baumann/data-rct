...
for arch in $ARCHS; do
  case "$arch" in
    x86_64)
      if [[ $IS_MAC == 1 ]]; then
        # Hardware macOS targets
        echo "Building for x86_64-apple-darwin"
        $HOME/.cargo/bin/cargo rustc -p "${FFI_TARGET}" --lib --crate-type staticlib $RELFLAG --target x86_64-apple-darwin
        exit 0
      fi
    ...
    arm64)
      if [[ $IS_MAC == 1 ]]; then
        ...
        $HOME/.cargo/bin/cargo rustc -p "${FFI_TARGET}" --lib --crate-type staticlib $RELFLAG --target aarch64-apple-darwin
      elif [ $IS_SIMULATOR -eq 0 ]; then
